...
* **Web GUI**: This tool provides Web-based GUI to the NDTwin user. The NDTwin user can use any web browser to run up this tool to easily view the real-time states of the network or control the network in real time. This tool supports LLM-powered intent-based network management interface.    
...
* **On-line LLM (e.g., ChatGPT)**: This tool refers to an on-line Large Language Model (LLM) website. The Web GUI tool supports the intent-based network management interface by intelligently prompting the on-line LLM (e.g., ChatGPT). 
...
* **Flow Information & Link Bandwidth Usage Collector**:  This module acts as a high-speed telemetry engine using the sFlow standard. It runs a non-blocking UDP listener to process two types of sFlow samples: (1) Flow Data: Extracts 5-tuple information (Src/Dst IP, Src/Dst Port, Protocol) in sampled packet headers to detect flows, know their routing paths, and calculate their current sending rates. (2) Link Data: Uses link counter samples to calculate real-time link bandwidth usage and the unused bandwidth of a link.
* **Topology & Flow Monitor**: This module maintains an in-memory graph representation of the network. It polls the SDN controller to detect topology changes (Link Recovery/Failure or Switch Join/Leave). Besides, it uses algorithms (like DFS/BFS) to calculate the routing path for any pair of hosts.
...
* **Device Configuration & Power Manager**: This module manages the physical state of network switches beyond standard OpenFlow capabilities: (1) Configuration: Retrieves device info via SNMP, (2) Power Control: Controls switch power status (On/Off) using APIs from intelligent smart plugs, enabling energy-saving applications.
* **Application Registration & Coordination Manager**: This module manages the lifecycle of NDTwin applications. It includes a conflict resolution locking mechanism to coordinate actions among different NDTwin applications, preventing conflicting network policies (e.g., one application tries to power off a switch while another application is trying to route traffic through it). 
...
* **Controller and Other Events Handler**: This module handles all events from the SDN controller, NDTwin applications, and NDTwin tools. Each event is a RESTful API request issued by one of these external processes and will be processed by a thread that is created on the fly to maximize the processing throughput.
* **Intent to Tasks Translator**: The Web GUI tool supports the intent-based network management interface by which the NDTwin user can use natural language to express their intents to manage the network. This module works with the Web GUI tool and will prompt an on-line LLM (e.g., ChatGPT) to translate the input intents to the corresponding tasks. To perform these translated tasks, this module will call the functions of related modules in the kernel.  
* **Data Cache Manager**: This module caches the states of network (e.g., the routing entries on a switch or the network topology) that change infrequently. When NDTwin applications or NDTwin tools issue requests to the kernel to retrieve the data, this module will provide the cached data without issuing commands to network switches to refetch the data. This design can provide the requested data to       
* **Data Cache Manager**: This module caches the states of network (e.g., the routing entries on a switch or the network topology) that change infrequently. When NDTwin applications or NDTwin tools issue requests to the kernel to retrieve the data, this module will provide the cached data without issuing commands to network switches to refetch the data. Instead, this module will issue refetch commands to network switches only when needed. This design provides the requested data to NDTwin applications and NDTwin tools very quickly and prevents network switches from overloading with frequent requests.       
...
## Required Resources Consideration
...
When using NDTwin to control a very large network with a huge number of flows, if the computational and storage resources of a server are insufficient for NDTwin to perform optimally, the NDTwin applications, tools, kernel, and SDN controller can run simultaneously on different servers as needed.  