...
* **Flow Information & Link Bandwidth Usage Collector**:  This module acts as a high-speed telemetry engine using the sFlow standard. It runs a non-blocking UDP listener to process two types of sFlow samples: (1) Flow Data: It extracts 5-tuple information (Src/Dst IP, Src/Dst Port, Protocol) in sampled packet headers to detect flows, know their routing paths, and calculate their current sending rates. (2) Link Data: It uses link counter samples to calculate real-time link bandwidth usage and the unused bandwidth on a link.
...
* **Device Configuration & Power Manager**: This module manages the physical states of network switches beyond standard OpenFlow capabilities: (1) For Configuration: This module retrieves switch configuration information via SNMP, (2) For Power Control: This module controls switch power status (On/Off) using APIs from intelligent smart plugs, enabling energy-saving applications.
* **Application Registration & Coordination Manager**: Each NDTwin application process should register with this module to get its unique run-time ID. This ID should be included in the requests issued by an NDTwin application to the kernel. Based on this ID, the kernel can asynchronously send back the replies to the correct NDTwin application process. By providing lock APIs, this module coordinates the actions among different NDTwin applications to prevent conflicting network policies from being executed (e.g., one NDTwin application tries to power off a switch to save energy consumption while another NDTwin application is using the same switch for traffic engineering). 
