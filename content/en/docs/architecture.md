...
* **Simulation Platform Manager**: This tool will launch a pre-installed and specified simulator program each time when it receives a simulation request issued by an NDTwin application. Because each received simulation request is executed by a launched simulator process, multiple simulations can run in parallel over multiple CPU cores to quickly find the best simulation result. 
