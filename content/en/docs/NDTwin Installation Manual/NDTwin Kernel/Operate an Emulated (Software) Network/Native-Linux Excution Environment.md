...
### Step 2.1: Create Ryu Environment (`ryu-env`)
...
pip install networkx==3.1 ovs==3.6.1 requests==2.32.4 loguru==0.7.3
```
...
* *Note: When installing the Python package ryu, the build process may fail during wheel generation with the following error:*

```bash
AttributeError: 'types.SimpleNamespace' object has no attribute 'get_script_args'
ERROR: Failed to build 'ryu' when getting requirements to build wheel
```
To resolve the issue, we commented out the problematic line in ryu/hooks.py:
```bash
...
This project uses a customized Ryu (OpenFlow 1.3) controller application to bootstrap routing and maintain topology awareness. When the controller starts and switches connect, it computes and installs all-destination routing entries (IPv4 destination-based forwarding rules) across the network to enable immediate end-to-end connectivity. The app supports both **static** and **dynamic** topology modes: it loads a predefined topology from a JSON file when available, and otherwise **discovers the topology dynamically** using Ryu’s topology events (switch/link discovery) and host learning via packet-in (e.g., ICMP). After topology is ready, it computes paths and pushes the corresponding flow entries to each switch.
...
**Action:** Create a file named `intelligent_router.py` in your `ryu/ryu/app` directory and paste the above code:

```bash
cd ryu/ryu/app
touch intelligent_router.py
nano intelligent_router.py
# paste the above code snippet to this file and save
```
---
